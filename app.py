...
def gerar_arte_ranking_png(hub, df_ranking, limite, tipo="melhores"):
    """Gera PNG premium no modelo futurista aprovado, sem mostrar prévia na tela."""
    try:
    ...
    if ofensores:
        titulo = "RANKING DE OFENSORES"
        subtitulo = "ROTAS COM BAIXA PERFORMANCE NO DIA"
        legenda = f"{hub}  •  Motoristas abaixo de {limite:.0f}% de conclusão"
        chamada = "PLANO DE AÇÃO • FOCO NA RECUPERAÇÃO • CONTATO PRIORITÁRIO"
        cor_neon = (255, 62, 72)
        cor_neon2 = (255, 132, 35)
        badge = "ALERTA"
        asc = True
    else:
        titulo = "RANKING DE MOTORISTAS"
        subtitulo = "ENTREGAS FINALIZADAS NO DIA"
        legenda = f"{hub}  •  Entregas finalizadas no dia"
        chamada = f"PARABÉNS AOS MOTORISTAS COM {limite:.0f}% OU MAIS DE CONCLUSÃO DENTRO DO HORÁRIO!"
        cor_neon = (255, 92, 18)
        cor_neon2 = (255, 201, 35)
        badge = "TOP"
        asc = False
    ...
    GAP = 16
    PCT_W = 220
    LIST_X = LEFT + PCT_W + 18
    LIST_W = W - LIST_X - RIGHT
    ROW_H = 30
    MIN_H = 76
    PAD_Y = 22
    FONT_NAME_SIZE = 23
    ...
    for perf, grupo in grupos:
        ...
        h = max(MIN_H, PAD_Y * 2 + rows * ROW_H)
        section_meta.append((perf, grupo, cols, rows, h))
        total_h += h + GAP

    H = max(1120, TOP + total_h + FOOTER + 24)

    img = Image.new("RGB", (W, H), (5, 11, 24))
    ...
    gd.ellipse((-380, -260, 620, 520), fill=(*cor_neon, 90))
    gd.ellipse((W - 650, -300, W + 360, 560), fill=(*cor_neon2, 80))
    gd.ellipse((W - 720, H - 520, W + 330, H + 260), fill=(*cor_neon, 48))
    glow = glow.filter(ImageFilter.GaussianBlur(60))
    img = Image.alpha_composite(img.convert("RGBA"), glow).convert("RGB")
    draw = ImageDraw.Draw(img)

    # Grid/linhas tecnológicas discretas
    for x in range(-H, W + H, 135):
        draw.line((x, 0, x + int(H * 0.26), H), fill=(22, 77, 116), width=1)
    for y in range(56, H, 86):
        draw.line((0, y, W, y), fill=(13, 35, 58), width=1)
    for x in range(60, W, 180):
        draw.line((x, 20, x + 70, 20), fill=(255, 96, 20), width=1)

    # Fontes
    f_title = _carregar_fonte_pil(76, True)
    f_sub = _carregar_fonte_pil(42, True)
    f_chip = _carregar_fonte_pil(25, True)
    f_date = _carregar_fonte_pil(43, True)
    f_msg = _carregar_fonte_pil(27, True)
    f_pct_big = _carregar_fonte_pil(55, True)
    f_pct_med = _carregar_fonte_pil(46, True)
    f_label = _carregar_fonte_pil(26, True)
    f_name = _carregar_fonte_pil(FONT_NAME_SIZE, True)
    f_pos = _carregar_fonte_pil(18, True)
    f_footer = _carregar_fonte_pil(28, True)
    f_small = _carregar_fonte_pil(22, True)

    def rounded_glow_box(x1, y1, x2, y2, radius, fill, outline, width=3, glow_alpha=95):
        layer = Image.new("RGBA", (W, H), (0, 0, 0, 0))
        ld = ImageDraw.Draw(layer)
        for off, alpha in [(10, glow_alpha // 4), (5, glow_alpha // 2), (0, glow_alpha)]:
            ld.rounded_rectangle((x1 - off, y1 - off, x2 + off, y2 + off), radius=radius + off, outline=(*outline, alpha), width=max(1, width))
        layer = layer.filter(ImageFilter.GaussianBlur(5))
        base = Image.alpha_composite(img.convert("RGBA"), layer)
        dd = ImageDraw.Draw(base)
        dd.rounded_rectangle((x1, y1, x2, y2), radius=radius, fill=fill, outline=outline, width=width)
    ...
    def draw_truck_icon(cx, cy, scale=1.0, color=(255, 255, 255)):
        s = scale
        draw.rounded_rectangle((cx - 38*s, cy - 18*s, cx + 18*s, cy + 16*s), radius=int(5*s), fill=color)
        draw.polygon([(cx + 18*s, cy - 8*s), (cx + 43*s, cy - 8*s), (cx + 55*s, cy + 16*s), (cx + 18*s, cy + 16*s)], fill=color)
        draw.ellipse((cx - 26*s, cy + 7*s, cx - 8*s, cy + 25*s), fill=(9, 14, 27))
        draw.ellipse((cx + 28*s, cy + 7*s, cx + 46*s, cy + 25*s), fill=(9, 14, 27))
        for i in range(3):
            draw.line((cx - 70*s - i*14*s, cy - 11*s + i*10*s, cx - 46*s - i*14*s, cy - 11*s + i*10*s), fill=color, width=max(1, int(4*s)))

    def draw_trophy(cx, cy, scale=1.0, metal="gold"):
        if metal == "silver":
            c1, c2, c3 = (245, 248, 255), (145, 155, 170), (75, 84, 96)
        elif metal == "bronze":
            c1, c2, c3 = (255, 161, 61), (189, 81, 17), (89, 38, 12)
        else:
            c1, c2, c3 = (255, 218, 62), (255, 132, 16), (102, 56, 0)
        s = scale
        # brilho atrás
        draw.ellipse((cx - 75*s, cy - 78*s, cx + 75*s, cy + 78*s), fill=(255, 167, 20))
        draw.ellipse((cx - 68*s, cy - 70*s, cx + 68*s, cy + 70*s), fill=(22, 18, 16))
        # alças
        draw.arc((cx - 100*s, cy - 50*s, cx - 30*s, cy + 42*s), 82, 280, fill=c2, width=int(11*s))
        draw.arc((cx + 30*s, cy - 50*s, cx + 100*s, cy + 42*s), 260, 98, fill=c2, width=int(11*s))
        # taça
        draw.pieslice((cx - 54*s, cy - 64*s, cx + 54*s, cy + 50*s), 0, 180, fill=c1)
        draw.rectangle((cx - 54*s, cy - 10*s, cx + 54*s, cy + 12*s), fill=c1)
        draw.polygon([(cx - 40*s, cy + 10*s), (cx + 40*s, cy + 10*s), (cx + 20*s, cy + 62*s), (cx - 20*s, cy + 62*s)], fill=c2)
        draw.rectangle((cx - 15*s, cy + 58*s, cx + 15*s, cy + 92*s), fill=c2)
        draw.rounded_rectangle((cx - 58*s, cy + 86*s, cx + 58*s, cy + 108*s), radius=int(8*s), fill=c3)
        draw.rounded_rectangle((cx - 78*s, cy + 106*s, cx + 78*s, cy + 132*s), radius=int(8*s), fill=(30, 24, 18))
        draw.text((cx - 16*s, cy - 30*s), "★", fill=c3, font=_carregar_fonte_pil(int(38*s), True))

    # Header estilo arte aprovada
    date_txt = agora_brasil().strftime("%d/%m/%Y")
    # Marca Shopee textual
    draw.rounded_rectangle((48, 50, 112, 116), radius=12, outline=cor_neon, width=5)
    draw.text((65, 56), "S", fill=cor_neon, font=_carregar_fonte_pil(48, True))
    draw.text((130, 61), "Shopee", fill=cor_neon, font=_carregar_fonte_pil(54, False))
    draw.rounded_rectangle((48, 143, 390, 210), radius=8, fill=(8, 14, 28), outline=(88, 64, 45), width=2)
    draw.text((70, 163), legenda, fill=(255, 255, 255), font=f_chip)

    # Ícone caminhão central
    draw.ellipse((W//2 - 56, 26, W//2 + 56, 138), outline=cor_neon, width=4, fill=(20, 15, 12))
    draw_truck_icon(W//2, 82, 0.8, cor_neon2)

    title_w = draw.textlength(titulo, font=f_title)
    draw.text(((W - title_w) / 2, 70), titulo, fill=(255, 255, 255), font=f_title)
    sub_w = draw.textlength(subtitulo, font=f_sub)
    draw.text(((W - sub_w) / 2, 155), subtitulo, fill=cor_neon, font=f_sub)

    # Data em placa neon
    plate = (W//2 - 250, 218, W//2 + 250, 286)
    draw.polygon([(plate[0]+32, plate[1]), (plate[2]-32, plate[1]), (plate[2], (plate[1]+plate[3])//2), (plate[2]-32, plate[3]), (plate[0]+32, plate[3]), (plate[0], (plate[1]+plate[3])//2)], fill=(10, 12, 22), outline=cor_neon2)
    draw.rectangle((plate[0]+34, plate[1], plate[2]-34, plate[3]), outline=cor_neon2, width=2)
    center_text(plate, date_txt, f_date, (255, 255, 255))

    # Selo circular direito
    ring = (W - 230, 38, W - 66, 202)
    draw.ellipse(ring, outline=cor_neon2, width=8)
    draw.ellipse((ring[0]+18, ring[1]+18, ring[2]-18, ring[3]-18), outline=cor_neon, width=3)
    center_text(ring, "OK" if not ofensores else "!", f_sub, cor_neon)

    draw.line((34, 322, W - 34, 322), fill=cor_neon, width=3)
    draw.line((34, 332, W - 34, 332), fill=(23, 65, 91), width=2)
    msg_w = draw.textlength(chamada, font=f_msg)
    draw.text(((W - msg_w) / 2, 338), chamada, fill=(255, 255, 255), font=f_msg)
    if not ofensores:
        # realce do texto da meta
        destaque = f"{limite:.0f}% OU MAIS"
        # simples, central em amarelo abaixo se couber
        pass

    y = TOP
    global_pos = 1
    for sec_i, (perf, grupo, cols, rows, sec_h) in enumerate(section_meta):
        x1, y1, x2, y2 = LEFT, y, W - RIGHT, y + sec_h
        # sombra e caixa externa
        draw.rounded_rectangle((x1 + 8, y1 + 10, x2 + 8, y2 + 10), radius=20, fill=(0, 0, 0))
        panel_fill = (9, 16, 30) if not ofensores else (22, 10, 17)
        draw.rounded_rectangle((x1, y1, x2, y2), radius=20, fill=panel_fill, outline=cor_neon2 if not ofensores else cor_neon, width=3)
        # separador do bloco percentual
        draw.polygon([(LIST_X-12, y1), (LIST_X+12, y1), (LIST_X-16, y2), (LIST_X-40, y2)], fill=(24, 28, 39), outline=(90, 90, 100))

        # bloco percentual
        if perf >= 100 and not ofensores:
            left_fill = (65, 42, 5)
            metal = "gold"
        elif perf >= 99 and not ofensores:
            left_fill = (45, 52, 62)
            metal = "silver"
        elif perf >= 98 and not ofensores:
            left_fill = (69, 32, 7)
            metal = "bronze"
        elif ofensores:
            left_fill = (83, 16, 27)
        ...
        if sec_h >= 210:
            draw_trophy(x1 + PCT_W//2, y1 + sec_h//2 + 12, 0.55, metal)
            bw = draw.textlength(badge, font=f_label)
            draw.text((x1 + (PCT_W - bw) / 2, y2 - 48), badge, fill=cor_neon2 if not ofensores else (255, 180, 180), font=f_label)
        else:
            bw = draw.textlength(badge, font=f_label)
            draw.text((x1 + (PCT_W - bw) / 2, y1 + sec_h - 42), badge, fill=cor_neon2 if not ofensores else (255, 180, 180), font=f_label)

        # nomes em colunas
        col_gap = 22
        col_w = (LIST_W - col_gap * (cols - 1) - 40) / cols
        for idx, row in grupo.iterrows():
            c = idx // rows
            r = idx % rows
            xx = LIST_X + 24 + c * (col_w + col_gap)
            yy = y1 + PAD_Y + r * ROW_H
            nome = _formatar_nome_ranking(row.get("Motorista", ""))
            nome = _texto_cortado_pil(draw, nome, f_name, col_w - 67)
            badge_color = (225, 75, 32) if not ofensores else (210, 47, 57)
            draw.rounded_rectangle((xx, yy - 1, xx + 43, yy + 25), radius=5, fill=badge_color)
            pos_txt = f"{global_pos:02d}"
            center_text((xx, yy - 1, xx + 43, yy + 25), pos_txt, f_pos, (255, 255, 255))
            draw.text((xx + 55, yy - 1), nome, fill=(255, 255, 255), font=f_name)
            global_pos += 1

        y += sec_h + GAP

    # Rodapé no estilo aprovado
    footer_y = H - FOOTER
    draw.polygon([(34, footer_y + 18), (W - 500, footer_y + 18), (W - 460, H - 26), (70, H - 26)], fill=(12, 21, 34), outline=(35, 98, 142))
    draw.polygon([(W - 470, footer_y + 18), (W - 35, footer_y + 18), (W - 90, H - 26), (W - 530, H - 26)], fill=cor_neon, outline=cor_neon2)
    draw.text((88, footer_y + 42), "◎  FOCO        ◷  PONTUALIDADE        ◇  COMPROMISSO        ▰  RESULTADOS", fill=(255, 255, 255), font=f_footer)
    draw.text((W - 425, footer_y + 42), "JUNTOS ENTREGAMOS MAIS!", fill=(255, 255, 255), font=f_footer)
    draw_truck_icon(W - 70, footer_y + 58, 0.55, (255, 255, 255))
